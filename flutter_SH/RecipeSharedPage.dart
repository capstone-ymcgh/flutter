...
class BoxListProvider extends ChangeNotifier {
  ...
  Future<void> loadBoxList() async {
    ...
    // 'recipeBoxList' 키로 저장된 JSON 문자열을 불러옴
    final data = prefs.getString('recipeBoxList');

    if (data != null) {
      // JSON 문자열을 디코딩하여 List<dynamic> 객체로 변환
      final List<dynamic> decodedData = jsonDecode(data);

      // List<dynamic> 객체를 List<BoxItem> 객체로 변환
                    ...
                    actions: [
                      ...
                      TextButton(
                        onPressed: () {
                          ...
                          boxListProvider.removeBox(widget.boxItem);
                          Navigator.of(context).pop(); // 다이얼로그 닫기
                          Navigator.of(context).pushAndRemoveUntil(
                            MaterialPageRoute(
                              builder: (context) => MyHomePage(initialIndex: 2), // 초기 인덱스를 2로 설정
                            ),
                                (route) => false,
                          );
                        },
          ...
          children: [
            SizedBox(
              ...
              child: IconButton(
                ...
                iconSize: 35,
                color: widget.boxItem.likeCount > 0 ? Colors.red : Colors.orange[300],
                padding: EdgeInsets.zero,
              ),
            ),
                    ...
                    children: [
                      OutlinedButton( // 학생, 성인 버튼 동작
                        ...
                        style: OutlinedButton.styleFrom(
                          side: BorderSide(
                            width: 1.0,
                            color: _selectedCategory == '학생' ? Colors.orange[300] ?? Colors.orange : Colors.grey,
                          ),
                          backgroundColor: _selectedCategory == '학생' ? Colors.orange[300] : Colors.transparent,
                          padding: EdgeInsets.symmetric(horizontal: 40.0), // 좌우 여백 조절
                        ),
                        child: Text(
                          '학생',
                          style: TextStyle(
                            color: _selectedCategory == '학생' ? Colors.white : Colors.orange[300], // 글자색 변경
                          ),
                        ),
                      ),
                      SizedBox(width: 15),
                      OutlinedButton(
                        ...
                        style: OutlinedButton.styleFrom(
                          side: BorderSide(
                            width: 1.0,
                            color: _selectedCategory == '성인' ? Colors.orange[300] ?? Colors.orange : Colors.grey,
                          ),
                          backgroundColor: _selectedCategory == '성인' ? Colors.orange[300] : Colors.transparent,
                          padding: EdgeInsets.symmetric(horizontal: 40.0), // 좌우 여백 조절
                        ),
                        child: Text(
                          '성인',
                          style: TextStyle(
                            color: _selectedCategory == '성인' ? Colors.white : Colors.orange[300], // 글자색 변경
                          ),
                        ),
                      ),
                    ],
                  ...
                  ElevatedButton(
                    onPressed: () {
                      ...
                      if (selectedImage == null) {
                        ...
                        // 상태 변경을 알리고 새로운 박스를 추가
                        final boxListProvider = Provider.of<BoxListProvider>(context, listen: false);
                        boxListProvider.addBox(newBoxItem);
