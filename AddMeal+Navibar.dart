  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        selectedItemColor: const Color(0xff58C58A),
        // 선택된 아이템의 아이콘과 라벨 색상 설정
        unselectedItemColor: const Color(0xff818582),
...
class Meal {
  ...
  // 객체 생성자 정의
  Meal({required this.title, required this.description,  this.imagePath,});
  ...
  Widget buildMainPage() {
    ...
    return Scaffold(
      appBar: AppBar(
        backgroundColor: Colors.white,
        title: const Padding(
          padding: EdgeInsets.only(left: 119, top: 0),
          child: Text(
            '식단 추가하기',
            ...
            child: GestureDetector(
              onTap: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => AddMealPage( //식단 작성페이지로 이동
                      onSave: (meal) {
                        addMeal(meal); //식단 작성 내용 저장
                        addBox(); //박스 추가하기
                      },
                    ),
                  ),
              ...
              if (boxes.isNotEmpty)
                Positioned.fill(
                  child: ListView.builder(
                    padding: const EdgeInsets.only(top: 12, bottom: 25.6), //2.2, 18.6
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          backgroundColor: Colors.white,
          title: const Padding(
            padding: EdgeInsets.only(left: 145.9, top: 0),
            child: Text(
              '선택 삭제',
          ...
          actions: <Widget>[
            IconButton(
              icon: const Icon(Icons.delete_outline), iconSize: 33,
              onPressed: () {
                showDialog(
                  context: context,
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  meal.description.length > 15 ? '${meal.description.substring(0, 15)}...' : meal.description,
...
class LongPressPopup extends StatelessWidget {
  ...
  const LongPressPopup({super.key, required this.onSelection, required this.child}); //생성자
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: GridView.count(
        ...
        mainAxisSpacing: 0.0, // Vertical spacing between grid items
        children: [
          Image.network(
...
class SecondPage extends StatefulWidget {
  ...
  const SecondPage({super.key, required this.meal});
...
class _SecondPageState extends State<SecondPage> {
  ...
  void initState() {
    ...
    if (widget.meal.imagePath != null) {
      selectedImage = File(widget.meal.imagePath!);
    }
  }
...
class AddMealPage extends StatefulWidget {
  ...
  const AddMealPage({Key? key, required this.onSave}) : super(key: key);
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                  decoration: const InputDecoration(labelText: '식단 제목',
                  ), // 기본 텍스트 색상 설정),
                  onChanged: (value) {
                    setState(() {
                      title = value;
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: TextField(
                        ...
                        decoration: InputDecoration(hintText: '작성할 내용을 입력하세요.'),
                        onChanged: (value) {
                          setState(() {
                            description = value;
                          });
              ...
              ElevatedButton(
                onPressed: () {
                  if (title.isEmpty || description.isEmpty) {
                    ...
                    // 제목과 설명이 모두 입력된 경우 박스 추가
                    Meal meal = Meal(title: title, description: description, imagePath: selectedImage?.path);
                    widget.onSave(meal); // 새로운 식단을 onSave 콜백을 통해 전달
                    Navigator.pop(context); // AddMealPage에서 뒤로 가기

                    // 추가 버튼을 눌렀을 때 title과 description 초기화
                    setState(() {
                      title = '';
                      description = '';
                      selectedImage = null;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        backgroundColor: Colors.white,
        title: const Padding(
          padding: EdgeInsets.only(left: 102.7, top: 0),
          child: Text(
            '알림창',
            style: TextStyle(fontSize: 25, color: Colors.black),
          ),
